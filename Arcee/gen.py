...
from transport2 import Sampler, create_transport
...
def main(mode, args):
    ...
    ckpt_path = args.ckpt
    inference_path = args.inference_dir
    os.makedirs(inference_path, exist_ok=True)
    # Auto-download a pre-trained model or load a custom SiT checkpoint from train.py:
    state_dict = find_model(ckpt_path, args.ema)
    ...
    transport = create_transport(args.path_type, args.prediction, args.loss_weight, args.sample_eps)
    ...
    # ------------------------------------------------------------------
    # Sample (grid_rows x grid_cols) images in small batches and save a single grid PNG
    # ------------------------------------------------------------------
    num_rows, num_cols = args.grid_rows, args.grid_cols
    num_total = num_rows * num_cols          # total images in the grid
    per_batch = min(args.global_batch_size, num_total)  # e.g. 4 on your GPU
    ...
    while num_generated < num_total:
        cur_bs = min(per_batch, num_total - num_generated)

        # fresh noise for this chunk
        z = torch.randn(cur_bs, 4, latent_size, latent_size, device=device)

        # labels (only if you’re using class conditioning)
        if use_label:
            # simple placeholder labels; change if you actually care about classes
            y = torch.zeros(cur_bs, dtype=torch.long, device=device)
        else:
            y = None
        ...
        if use_cfg:
            z = torch.cat([z, z], 0)
            if use_label:
                y_null = torch.full((cur_bs,), real_num_classes, dtype=torch.long, device=device)
                y = torch.cat([y, y_null], 0)
                model_kwargs = dict(y=y, cfg_scale=args.cfg_scale)
            else:
        ...
        # run sampler in latent space
        latents = sample_fn(z, model_fn, **model_kwargs)[-1]
        if use_cfg:
            # drop the null-guidance half
            latents, _ = latents.chunk(2, dim=0)

        # decode to RGB
        imgs = vae.decode(latents / 0.18215).sample   # (cur_bs, 3, H, W)
        all_samples.append(imgs.cpu())
        num_generated += imgs.size(0)

    # stack first num_total images and save as a num_rows x num_cols grid
    all_samples = torch.cat(all_samples, dim=0)[:num_total]
    print(f"Sampling took {time() - start_time:.2f} seconds.")

    grid_path = os.path.join(inference_path, f"qual_{num_rows}x{num_cols}.png")
    torchvision.utils.save_image(
        all_samples,
        grid_path,
        nrow=num_cols,                  # images per row
        normalize=True,
        value_range=(-1, 1),
        pad_value=1.0,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="path to checkpoint (default: auto-download a pre-trained SiT-XL/2 model).",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="path to a inference folder",
    )

    parser.add_argument("--model", type=str, default="Arcee-XS/2")
    parser.add_argument("--ema", action="store_true")
    parser.add_argument("--ssm-dstate", type=int, default=16, help="dstate for each d_inner of the SSM")
    parser.add_argument(
        "--scan-type",
        type=str,
        default="none",
    ...
    #irrelevant for sampling, but code trips if not set
