...
void set_ssm_params_fwd(SSMParamsBase &params,
    ...
    // h0 is const at::Tensor
    if (h0.defined()) {
        params.h0_ptr = h0.data_ptr();
        params.h0_batch_stride = h0.stride(0);
        params.h0_d_stride = h0.stride(1);
        params.h0_dstate_stride = h0.stride(2);
    } else {
        params.h0_ptr = nullptr;
    }

}

void set_ssm_params_bwd(SSMParamsBwd &params,
                        const at::Tensor h0,    // set h0 in params struct
                        // sizes
                        const size_t batch,
                        const size_t dim,
                        const size_t seqlen,
                        ...
                        // NOTE: surgery! Written by backward (gradient wrt initial state)
                        const at::Tensor &dh0,
                        // Consumed by backward (upstream gradient wrt returned last_state)
                        const at::Tensor &d_last_state
                    ) {
    // Pass in "dout" instead of "out", we're not gonna use "out" unless we have z
    set_ssm_params_fwd(params, h0, // now bwd params struct has h0
                       batch, dim, seqlen, dstate, n_groups, n_chunks, is_variable_B, is_variable_C,
    ...
    // dh0_ is required; kernel writes dL/dh0 into it
    if (dh0.defined()) {
        TORCH_CHECK(dh0.sizes() == h0.sizes(), "dh0 must have same shape as h0");
        TORCH_CHECK(dh0.scalar_type() == h0.scalar_type(), "dh0 must have same dtype as h0");
        params.dh0_ptr = dh0.data_ptr();
        params.dh0_batch_stride = dh0.stride(0);
        params.dh0_d_stride = dh0.stride(1);
        params.dh0_dstate_stride = dh0.stride(2);
    } else {
        params.dh0_ptr = nullptr;
    }

    // Set d_last_state ptrs in params struct
    if (d_last_state.defined()) {
        params.d_last_state_ptr = d_last_state.data_ptr();
        params.d_last_state_batch_stride = d_last_state.stride(0);
        params.d_last_state_d_stride = d_last_state.stride(1);
        params.d_last_state_dstate_stride = d_last_state.stride(2);
    } else {
        params.d_last_state_ptr = nullptr;
    }
}

std::vector<at::Tensor>
selective_scan_fwd(const c10::optional<at::Tensor> &h0_, // State initialization.
    ...
    // define and pass h0 to params struct if h0_.has_value() else pass undefined h0 tensor to params struct (set params handles rest)
    at::Tensor h0;
    if (h0_.has_value()) {
        h0 = h0_.value();
        TORCH_CHECK(h0.is_cuda(), "h0 must be on CUDA");
        CHECK_SHAPE(h0, batch_size, dim, dstate);

        if (!is_complex) {
            TORCH_CHECK(h0.scalar_type() == at::kFloat,
                        "For real weights (A.dtype=float), h0 must be float, got ", h0.scalar_type());
        } else {
            TORCH_CHECK(h0.scalar_type() == at::kComplexFloat,
                    "For complex weights (A.dtype=complex64), h0 must be complex64, got ", h0.scalar_type());
        }
    }
    ...
    set_ssm_params_fwd(params, h0, batch_size, dim, seqlen, dstate, n_groups, n_chunks, is_variable_B, is_variable_C,
    ...
    // Extract last hidden state: take last chunk (-1), and odd indices (1::2), even indices store running decay product and odd stores actual state
    at::Tensor last_state_view = x.index({Slice(),                // batch
                                    Slice(),                // dim
                                    -1,                     // last chunk
                                    Slice(1, None, 2)       // take state (skip running product of decays)
                                    });
    at::Tensor last_state = last_state_view.contiguous();
...
selective_scan_bwd (const at::Tensor &u, const at::Tensor &delta,
                ...
                const c10::optional<at::Tensor> &z_,
                const c10::optional<at::Tensor> &delta_bias_,
                // upstream gradient of output and last state render
                const at::Tensor &dout,
                const c10::optional<at::Tensor> &d_last_state_,
                const c10::optional<at::Tensor> &x_,
                const c10::optional<at::Tensor> &out_,
                c10::optional<at::Tensor> &dz_,
                bool delta_softplus,
                bool recompute_out_z,
                const c10::optional<at::Tensor> &h0_ // State initialization.
            ) {
    auto input_type = u.scalar_type();
    auto weight_type = A.scalar_type();
    TORCH_CHECK(input_type == at::ScalarType::Float || input_type == at::ScalarType::Half || input_type == at::ScalarType::BFloat16);
    ...
    // Upstream gradient wrt rendered last_state representation (optional)
    at::Tensor d_last_state;
    if (d_last_state_.has_value()) {
        d_last_state = d_last_state_.value();
        TORCH_CHECK(d_last_state.is_cuda(), "d_last_state must be CUDA");
        CHECK_SHAPE(d_last_state, batch_size, dim, dstate);
        if (weight_type == at::ScalarType::ComplexFloat) {
            TORCH_CHECK(d_last_state.scalar_type() == at::kComplexFloat, "For complex A, d_last_state must be complex64");
        } else {
            TORCH_CHECK(d_last_state.scalar_type() == at::kFloat, "For real A, d_last_state must be float32");
        }
    }
    ...
    // Allocate dh0 (gradient wrt initial state seed); kernel will write into it.
    at::Tensor dh0;
    if (h0_.has_value()) {dh0 = torch::zeros_like(h0_.value());}

    
    SSMParamsBwd params;
    set_ssm_params_bwd(params, 
        h0, // now bwd has access to h0 for mirroring fwd recurrence (idk if its properly setup though)
        batch_size, dim, seqlen, dstate, n_groups, n_chunks, is_variable_B, is_variable_C,
    ...
    if (!d_last_state_.has_value()) {
        TORCH_CHECK(params.d_last_state_ptr == nullptr, "d_last_state should be nullptr");
    }
    if (d_last_state_.has_value()) {
    TORCH_CHECK(params.d_last_state_ptr != nullptr, "d_last_state_ptr must be non-null when provided");
    }
    ...
    if (h0_.has_value()) {result.push_back(dh0);}
    return result;
}
